...
class FunctionDepartment extends React.Component{
    constructor(props) {
        super(props);
        this.state={
            list    : [],
...
//如果是搜索的话传入搜索和关键字
        ...
        _functiond.getFunctionDepartment(listParam).then(res => {  //this.state.curPage
    ...
    delPost(dpm_id){
         _functiond.getFunctionDepartmentDelete(this.state.dpm_id).then(res => {
            //console.log(res.data[0]);//取一个字段样例 去console里看
            alert('删除成功');
            this.loadUserList();
            //this.setState({totalPage: res.maxPage});
...
Deletedpm(e,dpm_id){
    ...
    render() {
        return (
            <div id="page-wrapper">
             <h1 className='page-header'>科室管理
                <div className="page-header-right">
                    <Link to ="/function_department/save" className='btn btn-primary'>
                            ...
                            this.state.list.map((dpm,index) => {
                                return(
                                    <tr key={index}>
                                         ...
                                         <td>{dpm.dpm_des}</td>
                                         <td> 
                                             <Link className="opear" to = { `/function_department/edit/${dpm.dpm_id}`}>编辑</Link>                                                      
                                         </td>
                                         <td>
                                            <button className="btn btn-xs btn-warning" onClick={(e) => {this.Deletedpm(e,dpm.dpm_id)}}>删除</button>
                                         </td>
                                    </tr>
                                 );
